...
export default function DeltaPerkasaApp() {
  // State untuk Hak Akses PIC: 'sales' | 'operator' | 'admin_timesheet' | 'management'
  ...
  // State Form Timesheet Operator/Admin
    ...
    <div className="min-h-screen bg-[#07090e] text-slate-100 p-4 md:p-8 font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-[#0d121f] border border-amber-500/30 rounded-3xl p-6 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-6">
          ...
          {/* TOMBOL PILIH HAK AKSES PIC */}
          <div className="flex flex-wrap items-center gap-1.5 bg-[#121824] p-1.5 border border-slate-800 rounded-2xl">
            <span className="text-[10px] font-bold text-slate-400 px-2">Pilih Akses PIC:</span>
            <button onClick={() => setCurrentRole('sales')} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${currentRole === 'sales' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}>📝 Sales</button>
            <button onClick={() => setCurrentRole('operator')} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${currentRole === 'operator' ? 'bg-purple-600 text-white' : 'text-slate-400 hover:text-white'}`}>🚜 Lapangan / Logistik</button>
            <button onClick={() => setCurrentRole('admin_timesheet')} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${currentRole === 'admin_timesheet' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}>📊 Admin Timesheet</button>
            <button onClick={() => setCurrentRole('management')} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${currentRole === 'management' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>👑 Management (All)</button>
          </div>
        </div>
        ...
        {(currentRole === 'sales' || currentRole === 'management') && (
          <div className="bg-[#0b0e17] border border-amber-500/30 rounded-3xl p-6 md:p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-lg font-black text-white">📝 Menu Khusus Sales: Form Sales Order</h2>
                <p className="text-xs text-slate-400">Input data sewa alat berat yang akan otomatis terkirim ke Kepala Operator & Logistik</p>
              </div>
            ...
            <form onSubmit={handleSalesSubmit} className="space-y-4">
              ...
              <button type="submit" className="w-full py-4 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl shadow-lg transition-all cursor-pointer">
        ...
        {/* 2. VIEW KHUSUS KEPALA OPERATOR / LAPANGAN & LOGISTIK */}
        {(currentRole === 'operator' || currentRole === 'management') && (
          <div className="bg-[#0b0e17] border border-purple-500/30 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
            <div>
              <h2 className="text-lg font-black text-white">🚜 Menu Kepala Operator & Logistik: Kontrol Lapangan & Unit</h2>
              <p className="text-xs text-slate-400">Pilih unit, tentukan operator, ambil foto muat/tiba, dan sinkronkan dengan Job ID</p>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#121824] border-b border-slate-800 text-[10px] text-slate-400 uppercase tracking-wider">
                    <th className="py-3 px-4">No. Order / Job ID</th>
        ...
        {/* 3. VIEW KHUSUS ADMIN TIMESHEET */}
        {(currentRole === 'admin_timesheet' || currentRole === 'management') && (
          <div className="bg-[#0b0e17] border border-emerald-500/30 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
            <div>
              <h2 className="text-lg font-black text-white">📊 Menu Admin Timesheet: Rekap Harian & Job ID</h2>
              <p className="text-xs text-slate-400">Catat Jam Kerja, HM Start, HM End, dan Sinkronisasi Tagihan Operasional</p>
            </div>
            ...
            <form onSubmit={handleTimesheetSubmit} className="space-y-4 bg-[#121824]/40 p-4 border border-slate-800 rounded-2xl">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <input type="text" name="kodeGajiOperator" value={timesheetForm.kodeGajiOperator} onChange={(e) => setTimesheetForm({...timesheetForm, kodeGajiOperator: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-slate-700 rounded-xl text-xs text-white" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Kode Tagih</label>
                  <input type="text" name="kodeTagih" value={timesheetForm.kodeTagih} onChange={(e) => setTimesheetForm({...timesheetForm, kodeTagih: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-slate-700 rounded-xl text-xs text-white" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-emerald-400 uppercase mb-1">Job ID</label>
                  <input type="text" name="jobId" value={timesheetForm.jobId} onChange={(e) => setTimesheetForm({...timesheetForm, jobId: e.target.value})} required className="w-full px-3 py-2 bg-[#0b0e17] border border-emerald-600 rounded-xl text-xs font-mono font-bold text-emerald-300" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Operator</label>
                  <select name="operator" value={timesheetForm.operator} onChange={(e) => setTimesheetForm({...timesheetForm, operator: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-slate-700 rounded-xl text-xs font-bold text-emerald-300">
                    {operatorDatabase.map(op => <option key={op} value={op}>{op}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <input type="text" name="hmStart" value={timesheetForm.hmStart} onChange={(e) => setTimesheetForm({...timesheetForm, hmStart: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-teal-800 rounded-xl text-xs font-mono font-bold text-teal-300" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-teal-400 uppercase mb-1">HM End</label>
                  <input type="text" name="hmEnd" value={timesheetForm.hmEnd} onChange={(e) => setTimesheetForm({...timesheetForm, hmEnd: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-teal-800 rounded-xl text-xs font-mono font-bold text-teal-300" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-amber-400 uppercase mb-1">Unit Code</label>
                  <input type="text" name="unitCode" value={timesheetForm.unitCode} onChange={(e) => setTimesheetForm({...timesheetForm, unitCode: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-slate-700 rounded-xl text-xs text-amber-300" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Nama Penyewa</label>
                  <input type="text" name="namaPenyewa" value={timesheetForm.namaPenyewa} onChange={(e) => setTimesheetForm({...timesheetForm, namaPenyewa: e.target.value})} className="w-full px-3 py-2 bg-[#0b0e17] border border-slate-700 rounded-xl text-xs text-white" />
                </div>
              </div>
            ...
            <div className="flex justify-between items-center pt-2">
              <h3 className="text-sm font-bold text-white">Tabel Spreadsheet Rekap Timesheet</h3>
