  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 flex flex-col items-center">
      ...
      <div className="max-w-6xl w-full space-y-8">
                  ...
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-4 px-4 space-y-2">
                        ...
                        <div>
                          <input type="text" value={order.hmAwal} onChange={(e) => updateHmAwal(order.id, e.target.value)} placeholder="Input HM Awal / Kondisi BBM" className="w-full text-xs font-mono px-3 py-1 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg outline-none" />
                        </div>
                      </td>

                      <td className="py-4 px-4 space-y-2">
                        <input type="text" value={order.lokasiAwal} onChange={(e) => updateLokasiAwal(order.id, e.target.value)} placeholder="Lokasi Asal / Pool..." className="w-full text-xs font-mono px-3 py-1 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg outline-none mb-1" />
                        <input type="text" value={order.lokasiTujuan} onChange={(e) => updateLokasiTujuan(order.id, e.target.value)} placeholder="Lokasi Pengantaran / Tujuan..." className="w-full text-xs font-mono px-3 py-1 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg outline-none mb-1" />
                        <input type="text" value={order.picPenerima} onChange={(e) => updatePicPenerima(order.id, e.target.value)} placeholder="Nama PIC & No HP Penerima..." className="w-full text-xs font-mono px-3 py-1 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg outline-none mb-2" />
                        
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="block text-[10px] text-slate-400 font-bold mb-0.5">Tronton Pengangkut</label>
                            <select value={order.trontonUnit} onChange={(e) => updateTrontonUnit(order.id, e.target.value)} className="w-full text-xs font-mono font-bold px-2 py-1 bg-slate-950 border border-purple-600/60 text-purple-300 rounded-lg outline-none cursor-pointer">
                              {trontonFleet.map((t) => (
                          ...
                          <div>
                            <label className="block text-[10px] text-slate-400 font-bold mb-0.5">Status Logistik</label>
                            <select value={order.statusLogistik} onChange={(e) => updateStatusLogistik(order.id, e.target.value)} className="w-full text-xs font-bold px-2 py-1 bg-slate-950 border border-purple-600/60 text-purple-300 rounded-lg outline-none cursor-pointer">
                              <option value="⏳ Menunggu Jadwal Muat">⏳ Menunggu Jadwal</option>
                              <option value="🏗️ Proses Muat di Pool">🏗️ Proses Muat</option>
                              <option value="🚚 Dalam Perjalanan (OTW)">🚚 OTW Lapangan</option>
                              <option value="✅ Tiba di Lokasi Proyek">✅ Tiba di Lokasi</option>
                            </select>
                          </div>
                        </div>

                        <input type="text" value={order.catatanLogistik} onChange={(e) => updateCatatanLogistik(order.id, e.target.value)} placeholder="Instruksi khusus logistik..." className="w-full text-xs font-mono px-3 py-1 bg-slate-950 border border-purple-600/40 text-purple-200 rounded-lg outline-none" />
                        
                        {/* FOTO & BUKTI GEOTAG */}
                        <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-800">
                          <div className="bg-slate-950 p-2 rounded-xl border border-slate-800">
                            <div className="flex justify-between items-center mb-1">
                              <span className="text-[10px] font-bold text-slate-400 uppercase">Foto Muat</span>
                              <button onClick={() => triggerCamera(order.id, 'muat')} className="px-2 py-0.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold rounded cursor-pointer">📷 Ambil</button>
                            </div>
                            {order.fotoMuatUrl ? (
                              <div className="space-y-1">
                                <img src={order.fotoMuatUrl} alt="Muat" className="w-full h-16 object-cover rounded border border-slate-700" />
                                <div className="text-[9px] font-mono text-slate-400 truncate">🕒 {order.timestampMuat}</div>
                                <div className="text-[9px] font-mono text-teal-400 truncate">📍 {order.koordinatMuat}</div>
                              </div>
                            ) : (
                              <div className="text-[10px] text-slate-600 italic py-1">Belum ada foto muat</div>
                            )}
                          </div>

                          <div className="bg-slate-950 p-2 rounded-xl border border-slate-800">
                            <div className="flex justify-between items-center mb-1">
                              <span className="text-[10px] font-bold text-slate-400 uppercase">Foto Tiba</span>
                              <button onClick={() => triggerCamera(order.id, 'tiba')} className="px-2 py-0.5 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold rounded cursor-pointer">📷 Ambil</button>
                            </div>
                            {order.fotoTibaUrl ? (
                              <div className="space-y-1">
                                <img src={order.fotoTibaUrl} alt="Tiba" className="w-full h-16 object-cover rounded border border-slate-700" />
                                <div className="text-[9px] font-mono text-slate-400 truncate">🕒 {order.timestampTiba}</div>
                                <div className="text-[9px] font-mono text-teal-400 truncate">📍 {order.koordinatTiba}</div>
                              </div>
                            ) : (
                              <div className="text-[10px] text-slate-600 italic py-1">Belum ada foto tiba</div>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 space-y-2 align-top">
                        <select value={order.status} onChange={(e) => {
                        ...
                        }} className="w-full text-xs font-bold px-3 py-1.5 bg-slate-950 border border-blue-600/60 text-blue-300 rounded-lg outline-none cursor-pointer">
                          ...
                          <option value="Sedang Berjalan di Lapangan">Sedang Berjalan di Lapangan</option>
                          <option value="Selesai Kontrak / Tarik Unit">Selesai Kontrak / Tarik Unit</option>
                        </select>

                        <div className="pt-2 space-y-1.5">
                          <button onClick={() => sendLogisticsWhatsApp(order)} className="w-full py-2 bg-purple-700 hover:bg-purple-600 text-white font-bold text-xs rounded-xl shadow transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                            <span>🚚</span> Kirim Instruksi ke Logistik
                          </button>
                          <button onClick={() => sendLogisticsUpdateToSales(order)} className="w-full py-2 bg-emerald-700 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                            <span>📢</span> Kirim Laporan ke Sales
                          </button>
                          <button onClick={() => sendWhatsAppNotification(order)} className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-xl border border-slate-700 transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                            <span>💬</span> Update Umum WA
                          </button>
                        </div>
                      </td>
                    </tr>
        ...
        {/* DATABASE ARMADA & MONITORING KONDISI UNIT */}
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <h2 className="text-xl font-black text-white">Database Armada & Status Kesiapan Unit</h2>
              <p className="text-xs text-slate-400">Pusat kontrol ketersediaan unit alat berat CV Chandra Delta Perkasa (Makassar & Sulawesi)</p>
            </div>

            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <input type="text" value={fleetSearchQuery} onChange={(e) => setFleetSearchQuery(e.target.value)} placeholder="Cari kode unit (Contoh: EXC.08, VBR)..." className="px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-100 outline-none focus:ring-2 focus:ring-blue-500 w-full md:w-64" />
            </div>
          </div>

          <div className="flex flex-wrap gap-1.5 mb-6 bg-slate-950 p-2 border border-slate-800 rounded-xl">
            {uniqueClasses.map(cls => (
              <button key={cls} onClick={() => setSelectedFleetFilter(cls)} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${selectedFleetFilter === cls ? 'bg-amber-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}>
                {cls}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 max-h-96 overflow-y-auto p-1">
            {filteredFleet.map((item) => {
              ...
              if (currentCondition === 'Working') badgeColor = 'bg-blue-950/80 text-blue-300 border-blue-500/40';
              if (currentCondition === 'Breakdown') badgeColor = 'bg-red-950/80 text-red-300 border-red-500/40';

              return (
                <div key={item.code} className="bg-slate-950 border border-slate-800 rounded-2xl p-3 flex flex-col justify-between hover:border-slate-700 transition-all">
                  <div>
                  ...
                  <div className="space-y-1 pt-2 border-t border-slate-900">
                    <select value={currentCondition} onChange={(e) => updateFleetCondition(item.code, e.target.value)} className="w-full text-[10px] font-bold px-2 py-1 bg-slate-900 border border-slate-700 text-slate-200 rounded-lg outline-none cursor-pointer">
